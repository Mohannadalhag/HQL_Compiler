...
/////////
funcpp : type L_IDENTCPP T_OPEN_P (paramcpp (T_COMMA paramcpp)*)? T_CLOSE_P bodycpp;
bodycpp : T_OPEN_B (stmtcpp|bodycpp)*  end_funcpp;
end_funcpp : T_CLOSE_B;
returncpp : T_RETURN exprcpp? T_SEMICOLON;
type : T_CHAR|T_DOUBLE|T_INT|T_FLOAT|T_BOOLEAN|T_VOID;
paramcpp : type L_IDENTCPP (T_EQUAL numbercpp)?;
numbercpp : int_number|dec_number;
stmtcpp : returncpp|assign|stmt_if_cpp|stmt_for_cpp|select_stmt|create_table_stmt;
assign :(assign_select | assign_query_select |assign_with_type|assign_without_type) end_assign;
end_assign : T_SEMICOLON;
assign_select : L_IDENTCPP T_EQUAL select_stmt;
assign_query_select : L_IDENTCPP T_EQUAL T_QUERY select_stmt;
assign_with_type : type L_IDENTCPP eqexprcpp? ;
eqexprcpp : T_EQUAL exprcpp;
assign_without_type : L_IDENTCPP assign_without_type2;
assign_without_type2 : (T_ADD (T_ADD|eqexprcpp))|
                     (T_SUB (T_SUB|eqexprcpp))|
                     eqexprcpp;
stmt_if_cpp : T_IF T_OPEN_P bool_expr_cpp T_CLOSE_P bodycpp
             (T_ELSE T_IF T_OPEN_P bool_expr_cpp T_CLOSE_P bodycpp)*
             (T_ELSE bodycpp)?;
stmt_for_cpp : T_FOR stmt_for_body_cpp bodycpp;
stmt_for_body_cpp : T_OPEN_P (assign_with_type|assign_without_type) (T_COMMA(assign_with_type|assign_without_type))* T_SEMICOLON bool_expr_cpp?
                  T_SEMICOLON (assign_without_type)? (T_COMMA assign_without_type)* T_CLOSE_P;
bool_expr_cpp : T_NOT? exprcpp (comparecpp T_NOT? exprcpp )? bool_expr_cpp2?;
bool_expr_cpp2 : (T_AND2|T_PIPE)bool_expr_cpp;
comparecpp : T_EQUAL2|T_GREATEREQUAL|T_GREATER|T_LESSEQUAL|T_LESS|T_NOTEQUAL2;
exprcpp : (exprcppt plusormenus exprcpp)|exprcppt;
exprcppt : identexprcpp
        |(identexprcpp (T_MUL|T_DIV) exprcppt)|(T_OPEN_P exprcpp T_CLOSE_P);
identexprcpp : plusormenus2?(L_IDENTCPP|numbercpp) plusormenus2?;
plusormenus2 : (T_ADD T_ADD)|(T_SUB T_SUB);
plusormenus : T_ADD|T_SUB;
...
exception_block_item :
       T_WHEN (L_ID|L_IDENTCPP) T_THEN block ~(T_WHEN | T_END)
     ;
...
create_table_columns_error :(T_SEMICOLON create_table_columns_item);
create_table_columns :
       create_table_columns_item ((T_COMMA create_table_columns_item)|create_table_columns_error)*
     ;
...
dtype_len_error:L_DEC;
dtype_len :             // Data type length or size specification
       T_OPEN_P ((L_INT|dtype_len_error | T_MAX) (T_CHAR | T_BYTE )? (T_COMMA (L_INT|dtype_len_error))?  )T_CLOSE_P
     ;
...
create_database_stmt_error:(T_IF T_NOT T_EXISTS_ERROR);
create_database_stmt :
      T_CREATE (T_DATABASE | T_SCHEMA) ((T_IF T_NOT T_EXISTS)?|(T_IF T_NOT T_EXISTS_ERROR)?) expr create_database_option*
    ;
...
create_function_stmt :
      (T_ALTER | T_CREATE (T_OR T_REPLACE)? | T_REPLACE)? T_FUNCTION ident create_routine_params?
      create_function_return (T_AS | T_IS)? declare_block_inplace? single_block_stmt
    ;
...
for_cursor_stmt :       // FOR (cursor) statement
       T_FOR (L_ID|L_IDENTCPP) T_IN T_OPEN_P? select_stmt T_CLOSE_P? T_LOOP block T_END T_LOOP
     ;

for_range_stmt_error:;
for_range_stmt :        // FOR (Integer range) statement
       T_FOR (L_ID|L_IDENTCPP) (T_IN|for_range_stmt_error) T_REVERSE? expr T_DOT2 expr ((T_BY | T_STEP) expr)? T_LOOP block T_END T_LOOP
     ;


label :
       L_LABEL
     | T_LESS T_LESS (L_ID|L_IDENTCPP) T_GREATER T_GREATER
     ;
...
fullselect_stmt_item_error:T_SEMICOLON+;
fullselect_stmt_item :
       (subselect_stmt
     | T_OPEN_P fullselect_stmt T_CLOSE_P)fullselect_stmt_item_error?
     ;
...
select_list_asterisk :
       ((L_ID|L_IDENTCPP) '.')? '*'|select_list_asterisk_error
     ;
...
from_alias_clause :
       ...
       T_AS? ident (T_OPEN_P (L_ID|L_IDENTCPP) (T_COMMA (L_ID|L_IDENTCPP))* T_CLOSE_P)?
     ;
...
expr_error:T_SUB|T_ADD|T_MUL|T_DIV|'%';
expr :
       expr interval_item
     | expr T_MUL (expr|expr_error)*///
     | expr T_DIV (expr|expr_error)*///
     | expr T_ADD (expr|expr_error)*///
     | expr T_SUB (expr|expr_error)*///
...
expr_atom_error:int_number(T_COMMA int_number);
expr_atom :
       date_literal/////
     | timestamp_literal
     | bool_literal//////
...
ident :
       (L_IDENTCPP|(L_ID|L_IDENTCPP) | non_reserved_words) ('.' (L_IDENTCPP|(L_ID|L_IDENTCPP) | non_reserved_words))*
     ;
...
L_IDENTCPP:[a-zA-Z] ([a-zA-Z] | L_DIGIT | '_')*;
...
L_FILE      : ([a-zA-Z] ':' '\\'?)? (L_ID|L_IDENTCPP) ('\\' (L_ID|L_IDENTCPP))*                  // File path (a/b/c Linux path causes conflicts with division operator and handled at parser level)
            ;
